...
<Td className="border-0">
				...
				Contains return progress. The completion event uses `status: COMPLETED`; do not treat `REFUNDING` as completed. Match `returnId`/`returnTrackId` to the return and `orderId`/`orderTrackId` to the original order. See [Return Request API](/api-list/return-request) for the full status flow.
